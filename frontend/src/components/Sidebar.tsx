    ...
    <div className={`${isCollapsed ? 'w-16' : 'w-64'} transition-all duration-300 bg-white shadow-lg border-r border-gray-200 flex flex-col h-screen fixed left-0 top-0 z-50 hidden md:flex`}>
      ...
      <nav className="flex-1 py-4 px-2 overflow-y-auto">
        <div className="space-y-1">
          {navItems.map((item) => (
            ...
            >
              <span className={`transition-transform duration-200 group-hover:scale-110 ${isCollapsed ? '' : 'mr-3'}`}>
                {item.icon}
              </span>
              {!isCollapsed && (
      ...
      <div className="border-t border-gray-200 p-4">
        ...
        <button
          ...
          className={`flex items-center w-full px-3 py-2 text-sm font-medium text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-all duration-200 ${
            isCollapsed ? 'justify-center' : ''
          }`}
