...
class Supplier(models.Model):
    ...
    payment_terms = models.CharField(
        max_length=100, blank=True, verbose_name="付款條件"
    )
...
class Product(models.Model):
    ...
    dimensions = models.CharField(
        max_length=50, blank=True, verbose_name="尺寸(長x寬x高)"
    )
    ...
    def profit_margin(self):
        ...
        if self.cost_price > 0:
            return (
                (self.base_price - self.cost_price) / self.cost_price * 100
...
class ProductVariant(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name="variants", verbose_name="產品"
    )
...
class Inventory(models.Model):
    ...
    product = models.OneToOneField(
        Product, on_delete=models.CASCADE, related_name="inventory", verbose_name="產品"
    )
...
class StockMovement(models.Model):
    ...
    movement_type = models.CharField(
        max_length=20, choices=MOVEMENT_TYPES, verbose_name="異動類型"
    )
    quantity = models.IntegerField(verbose_name="異動數量")  # 可以為負數
    reference_type = models.CharField(
        max_length=20, choices=REFERENCE_TYPES, verbose_name="關聯類型"
    )
    reference_id = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="關聯ID"
    )
...
class PriceHistory(models.Model):
    ...
    def price_change_percentage(self):
        """價格變動百分比"""
        if self.old_price > 0:
            return ((self.new_price - self.old_price) / self.old_price * 100).quantize(
                Decimal("0.01")
            )
        return Decimal("0.00")
