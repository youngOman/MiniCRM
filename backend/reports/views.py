...
def dashboard_stats(request):
    ...
    stats = {
        'overview': {
            ...
            'average_order_value': float(orders_qs.aggregate(Avg('total'))['total__avg'] or 0),  # 計算特定時間範圍，所有訂單的平均訂單價值
        ...
        'customer_stats': {
            ...
            'avg_order_value': float(orders_qs.aggregate(Avg('total'))['total__avg'] or 0),  # 計算該客戶的平均訂單價值
            'avg_purchase_frequency': float(calculate_avg_purchase_frequency(customers_qs)),  # 平均購買頻率
...
def calculate_avg_clv(customers_qs):
    ...
    # 該期間總購買金額 ÷ 該期間總訂單數
    total_revenue = Order.objects.filter(customer__in=customers_with_orders).aggregate(Sum('total'))['total__sum'] or 0
    ...
    for customer in customers_with_orders:
        ...
        if customer_orders.count() > 1:  # 至少要有2筆訂單才能計算壽命
            ...
            lifespan_days = (last_order.order_date.date() - first_order.order_date.date()).days
    ...
    avg_customer_lifespan_months = (total_lifespan_days / lifespan_customer_count / 30) if lifespan_customer_count > 0 else 3
    ...
    print(f"  平均顧客壽命: {avg_customer_lifespan_months:.2f} 個月 ({avg_customer_lifespan_years:.2f} 年)")
...
def customer_clv_analytics(request):
    ...
    for customer in customers_qs.filter(orders__isnull=False).distinct():
        # 使用客戶總消費額作為個人價值指標
        customer_total_spent = customer.orders.aggregate(total=Sum('total'))['total'] or 0
    ...
    for source, data in source_clv_dict.items():
        ...
        # 計算該來源的真正 CLV（生命週期價值）
        source_customers_qs = customers_qs.filter(source=source, orders__isnull=False).distinct()
    ...
    for customer_id, total_spent in sorted_customers:
        try:
            ...
            customer_data = {
                ...
                'avg_order_value': float(total_spent) / customer_orders if customer_orders > 0 else 0
            }
