...
class CRMKnowledgeBase:
    ...
    def __init__(self, persist_directory: str = "chroma_db") -> None:
        ...
        )  # ChromaDB 進行向量儲存和相似度搜尋
        self.model = SentenceTransformer("all-MiniLM-L6-v2")  # 將文本轉換為向量

        # 初始化集合
        self.schema_collection = self._get_or_create_collection(
            "crm_schema"
        )  # 儲存資料庫 schema 資訊
        self.examples_collection = self._get_or_create_collection(
            "query_examples"
        )  # 儲存查詢範例
    ...
    def add_query_example(
        ...
        example_id = (
            f"example_{len(self.examples_collection.get()['ids'])}"  # 生成唯一 ID
        )
