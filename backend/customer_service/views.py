...
class ServiceTicketViewSet(viewsets.ModelViewSet):
    ...
    def add_note(self, request, pk=None):
        ...
        serializer = ServiceNoteSerializer(
        ...
        if serializer.is_valid():
            ...
            return Response(
                ServiceNoteSerializer(note).data, status=status.HTTP_201_CREATED
            )
    ...
    def stats(self, request):
        ...
        month_tickets = ServiceTicket.objects.filter(
        ...
        # 按優先級統計
        priority_stats = (
            ServiceTicket.objects.values("priority")
        ...
        # 按分類統計
        category_stats = (
            ServiceTicket.objects.values("category")
            .annotate(count=Count("id"))
            .order_by("-count")[:5]
        )
...
class FAQViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return FAQ.objects.filter(is_active=True).select_related(
            "category", "created_by"
        )
