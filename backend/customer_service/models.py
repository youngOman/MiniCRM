...
class ServiceTicket(models.Model):
    ...
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='service_tickets', verbose_name='客戶')
    title = models.CharField(max_length=200, verbose_name='工單標題')
    description = models.TextField(verbose_name='問題描述')
    
    # 分類和優先級
    category = models.CharField(max_length=50, choices=CATEGORY_CHOICES, default='general', verbose_name='問題分類')
    ...
    # 處理資訊
    assigned_to = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='assigned_tickets', verbose_name='負責人員')
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='created_tickets', verbose_name='建立人員')
    
    # 時間記錄
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='建立時間')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新時間')
    first_response_at = models.DateTimeField(null=True, blank=True, verbose_name='首次回應時間')
    ...
    satisfaction_rating = models.IntegerField(null=True, blank=True, verbose_name='滿意度評分', help_text='1-5分')
...
class ServiceNote(models.Model):
    ...
    ticket = models.ForeignKey(ServiceTicket, on_delete=models.CASCADE, related_name='notes', verbose_name='工單')
    content = models.TextField(verbose_name='記錄內容')
    note_type = models.CharField(max_length=20, choices=NOTE_TYPE_CHOICES, default='internal', verbose_name='記錄類型')
    
    # 作者資訊
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name='記錄人員')
...
class KnowledgeBaseCategory(models.Model):
    ...
    parent = models.ForeignKey('self', on_delete=models.CASCADE, null=True, blank=True, related_name='children', verbose_name='上級分類')
...
class KnowledgeBase(models.Model):
    ...
    # 分類和標籤
    category = models.ForeignKey(KnowledgeBaseCategory, on_delete=models.SET_NULL, null=True, blank=True, verbose_name='分類')
    content_type = models.CharField(max_length=20, choices=CONTENT_TYPE_CHOICES, default='faq', verbose_name='內容類型')
    tags = models.JSONField(default=list, blank=True, verbose_name='標籤')
    ...
    # 作者和時間
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='knowledge_articles', verbose_name='建立人員')
    updated_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='updated_knowledge_articles', verbose_name='更新人員')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='建立時間')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新時間')
