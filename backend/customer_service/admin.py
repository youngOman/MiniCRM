...
class ServiceTicketAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基本資訊', {
            'fields': ('ticket_number', 'customer', 'title', 'description')
        }),
        ('分類和優先級', {
...
class ServiceNoteAdmin(admin.ModelAdmin):
    list_display = ['ticket', 'note_type', 'created_by', 'created_at', 'is_visible_to_customer']
...
class KnowledgeBaseAdmin(admin.ModelAdmin):
    list_display = [
        'title', 'category', 'content_type', 'is_public', 
...
class FAQAdmin(admin.ModelAdmin):
    list_display = ['question', 'category', 'is_featured', 'is_active', 'view_count', 'created_at']
