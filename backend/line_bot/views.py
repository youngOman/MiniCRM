...
def show_loading_animation_sync(user_id, loading_seconds=10):
    ...
    # 在背景執行緒中運行
    threading.Thread(target=run_async_loading, daemon=True).start() # 使用 daemon=True 確保主執行緒結束時，此執行緒也會被強制結束
...
請直接輸入您的問題，或使用下方快速選單！"""
    ...
    try:
        ...
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text="成功收到您的訊息嘍！")
        )
    ...
    processing_thread = threading.Thread(
        target=process_user_query_async,
        args=(user_id, user_message, event.reply_token)
    )
