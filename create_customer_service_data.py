...
class CustomerServiceDataGenerator:
    ...
    def get_or_create_admin_user(self):
        ...
        try:
            # 檢查是否有超級用戶
            self.cursor.execute("SELECT id FROM auth_user WHERE is_superuser = 1 LIMIT 1")
    ...
    def create_test_customers(self):
        ...
        customers_data = [
            {'name': '王小明', 'email': 'wang.xiaoming@test.com', 'phone': '0912-345-678'},
            {'name': '李小華', 'email': 'li.xiaohua@test.com', 'phone': '0923-456-789'},
            {'name': '陳大文', 'email': 'chen.dawen@test.com', 'phone': '0934-567-890'},
            {'name': '林小美', 'email': 'lin.xiaomei@test.com', 'phone': '0945-678-901'},
            {'name': '張志明', 'email': 'zhang.zhiming@test.com', 'phone': '0956-789-012'},
            {'name': '劉小芳', 'email': 'liu.xiaofang@test.com', 'phone': '0967-890-123'},
            {'name': '黃大明', 'email': 'huang.daming@test.com', 'phone': '0978-901-234'},
            {'name': '吳小玲', 'email': 'wu.xiaoling@test.com', 'phone': '0989-012-345'},
            {'name': '周文華', 'email': 'zhou.wenhua@test.com', 'phone': '0990-123-456'},
            {'name': '蔡小雯', 'email': 'cai.xiaowen@test.com', 'phone': '0901-234-567'},
            {'name': '楊志強', 'email': 'yang.zhiqiang@test.com', 'phone': '0912-345-670'},
            {'name': '許美玲', 'email': 'xu.meiling@test.com', 'phone': '0923-456-781'},
            {'name': '郭大偉', 'email': 'guo.dawei@test.com', 'phone': '0934-567-892'},
            {'name': '謝小君', 'email': 'xie.xiaojun@test.com', 'phone': '0945-678-903'},
            {'name': '高志文', 'email': 'gao.zhiwen@test.com', 'phone': '0956-789-014'},
            {'name': '蘇小慧', 'email': 'su.xiaohui@test.com', 'phone': '0967-890-125'},
            {'name': '江大成', 'email': 'jiang.dacheng@test.com', 'phone': '0978-901-236'},
            {'name': '賴小英', 'email': 'lai.xiaoying@test.com', 'phone': '0989-012-347'},
            {'name': '簡志豪', 'email': 'jian.zhihao@test.com', 'phone': '0990-123-458'},
            {'name': '范小珍', 'email': 'fan.xiaozhen@test.com', 'phone': '0901-234-569'},
        ]
        ...
        for data in customers_data:
            ...
            try:
                self.cursor.execute(insert_sql, (
                    first_name, last_name, data['email'], data['phone'],
                    random.choice(['台北', '新北', '桃園', '台中', '台南', '高雄']),
                    '台灣',
                    random.choice(['website', 'social_media', 'referral', 'advertisement']),
                    random.randint(20, 65),
                    random.choice(['male', 'female']),
                    self.admin_user_id, now, now
                ))
        ...
        if not self.customers:
            self.cursor.execute("SELECT id, first_name, last_name, email FROM customers_customer LIMIT 20")
    ...
    def create_knowledge_categories(self):
        ...
        if not self.categories:
            self.cursor.execute("SELECT id, name FROM customer_service_knowledgebasecategory")
            for row in self.cursor.fetchall():
    ...
    def create_service_tickets(self, count=50):
        ...
        title_templates = [
            "無法登入帳戶", "付款失敗問題", "產品功能異常", "訂單狀態查詢", "退款申請",
            "密碼重設問題", "商品配送延遲", "帳單金額疑問", "會員等級問題", "系統操作困難",
            "產品品質問題", "服務態度投訴", "功能建議", "優惠券使用問題", "個人資料修改",
            "訂閱取消申請", "技術支援需求", "商品退換貨", "發票開立問題", "系統錯誤回報"
        ]
        ...
        created_count = 0
        categories = ['general', 'technical', 'billing', 'product', 'shipping', 'return', 'complaint', 'feature_request']
        ...
        for i in range(count):
            ...
            if status in ['in_progress', 'pending', 'resolved', 'closed']:
                first_response_at = (created_time + timedelta(hours=random.randint(1, 24))).strftime('%Y-%m-%d %H:%M:%S')
                
            if status in ['resolved', 'closed']:
                resolved_at = (created_time + timedelta(hours=random.randint(25, 72))).strftime('%Y-%m-%d %H:%M:%S')
                
            if status == 'closed':
                closed_at = (created_time + timedelta(hours=random.randint(73, 96))).strftime('%Y-%m-%d %H:%M:%S')
            
            tags = json.dumps(random.sample(['緊急', '重要', '新客戶', '老客戶', 'VIP', '技術', '計費'], k=random.randint(0, 3)))
            ...
            try:
                self.cursor.execute(insert_sql, (
                    ...
                    resolved_at,
                    closed_at,
                    random.randint(1, 5) if status in ['resolved', 'closed'] and random.choice([True, False]) else None,
                    "客戶滿意服務品質" if status in ['resolved', 'closed'] and random.choice([True, False]) else None,
                ...
                # 為非開啟狀態的工單創建服務記錄
                if status != 'open':
                    self.create_service_notes_for_ticket(ticket_id, status, created_time)
    ...
    def create_service_notes_for_ticket(self, ticket_id, status, created_time):
        ...
        for i in range(num_notes):
            ...
            note_time = (created_time + timedelta(hours=hours_after)).strftime('%Y-%m-%d %H:%M:%S')
    ...
    def create_knowledge_base_articles(self, count=50):
        ...
        articles_data = [
            {"title": "如何重設密碼", "summary": "詳細說明密碼重設的步驟和注意事項", "content_type": "guide"},
            {"title": "會員等級說明", "summary": "介紹不同會員等級的權益和升級條件", "content_type": "policy"},
            {"title": "付款方式設定", "summary": "支援的付款方式和設定流程", "content_type": "guide"},
            {"title": "訂單狀態查詢", "summary": "如何查詢和追蹤訂單處理進度", "content_type": "guide"},
            {"title": "退換貨政策", "summary": "退換貨的條件、流程和相關規定", "content_type": "policy"},
        ]
        
        additional_titles = [
            "購物車使用技巧", "商品搜尋功能", "個人資料管理", "通知設定調整",
            "手機版操作指南", "桌面版功能介紹", "新手入門教學", "進階功能使用",
            "常見錯誤處理", "網路連線問題", "瀏覽器相容性", "APP安裝指南"
        ]
        ...
        for i in range(count):
            if i < len(articles_data):
                ...
                title = random.choice(additional_titles + [f"技術文檔 #{i-len(articles_data)+1}"])
...
"""
            ...
            tags = json.dumps(random.sample(['教學', '指南', '政策', '技術', '新手', '進階'], k=random.randint(1, 4)))
            category_id = random.choice(self.categories)['id'] if self.categories else None
            now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            
            try:
                self.cursor.execute(insert_sql, (
                    title, content, summary, category_id, content_type, tags,
                    1, random.choice([True, False, False, False]),  # 25% 精選
                    1, random.randint(0, 1000), random.randint(0, 100), random.randint(0, 20),
                    self.admin_user_id, self.admin_user_id, now, now
                ))
                created_count += 1
                if created_count % 10 == 0:
    ...
    def create_faq_entries(self, count=50):
        ...
        faq_data = [
            {"question": "如何註冊新帳戶？", "answer": "點擊頁面右上角的「註冊」按鈕，填寫電子郵件、設定密碼，然後點擊確認即可完成註冊。"},
            {"question": "忘記密碼怎麼辦？", "answer": "在登入頁面點擊「忘記密碼」，輸入註冊時的電子郵件，系統會發送重設密碼的連結到您的信箱。"},
            {"question": "如何修改個人資料？", "answer": "登入後進入「會員中心」→「個人資料」，即可修改姓名、電話、地址等資訊。"},
            {"question": "支援哪些付款方式？", "answer": "我們支援信用卡、LINE Pay、Apple Pay、Google Pay、ATM轉帳和貨到付款等多種付款方式。"},
            {"question": "如何查詢訂單狀態？", "answer": "登入會員後進入「訂單管理」，即可查看所有訂單的詳細狀態和物流資訊。"},
        ]
        
        additional_questions = [
            ("客服服務時間？", "客服服務時間為週一至週五 09:00-18:00，週六 09:00-12:00，週日及國定假日休息。"),
            ("如何聯繫客服？", "可透過線上客服、客服電話 02-1234-5678 或電子郵件 service@example.com 聯繫我們。"),
            ("商品有保固嗎？", "大部分商品都有製造商提供的保固服務，詳細保固條款請參考商品頁面說明。"),
            ("可以取消訂單嗎？", "訂單在「待出貨」狀態下可以取消。已出貨的訂單請改走退貨流程。"),
        ]
        
        all_questions = faq_data + [{"question": q, "answer": a} for q, a in additional_questions]
        ...
        for i in range(count):
            ...
            try:
                self.cursor.execute(insert_sql, (
                    question, answer, category_id, 1,
                    random.choice([True, False, False, False]),  # 25% 置頂
                    i * 10, random.randint(0, 500),
                    self.admin_user_id, now, now
                ))
                created_count += 1
                if created_count % 10 == 0:
    ...
    def run(self):
        ...
        try:
            ...
            self.cursor.execute("SELECT COUNT(*) FROM customer_service_knowledgebasecategory")
