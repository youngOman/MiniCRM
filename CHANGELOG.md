...
- **完整的客服工單管理系統**
  ...
  - 自動工單號碼生成：CS + 年月日 + 4位序號 (例：CS202507200001)
...
- **Django Admin 配置**：完整的後台管理界面
- **權限控制**：基於 Django 用戶權限系統
- **資料匯出**：支援各類報表匯出

### 為 AI + RAG 功能奠定基礎

- **豐富的業務知識庫**：為後續 AI 智慧查詢提供資料基礎
- **結構化資料**：便於 AI 理解和檢索的資料格式
- **完整的客服歷史**：支援上下文相關的 AI 回應

- **LINE + RAG 智慧查詢系統開發流程**，例：「幫我找出上個月購買最多的 25-35 歲女性客戶」
  ...
  - 收集公司文件（SOP、FAQ、產品知識庫、客服記錄）
  - 選擇向量資料庫（Chroma/Pinecone/Qdrant）
  - 文件切塊和向量化處理
  ...
  - 設計 Prompt Engineering 模板
  - 整合 LLM API（OpenAI/Claude/本地 Ollama）
  - 實現 Text-to-SQL 轉換器
  - 建立查詢意圖識別
  ...
  - 建立 AI Assistant Django App
  - 實現 LINE Webhook 處理器
  - 整合 RAG 和 Text-to-SQL 功能
  - 建立對話狀態管理
  ...
  ***
  ...
  - 雲端: OpenAI GPT-4o-mini / Claude 3.5 Haiku
  - 本地: Ollama + Qwen2.5-Coder

  向量資料庫

  - 輕量: Chroma (SQLite-based)
  - 企業: Pinecone / Qdrant
  ...
  ***
  ...
  用戶在 LINE: "幫我找出上個月購買最多的 25-35 歲女性客戶"
